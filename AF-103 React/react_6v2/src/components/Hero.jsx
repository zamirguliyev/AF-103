...
import { getAllData, deleteData,postData, getAlbumData, editData } from "../request.js";
...
export default function Hero() {
  ...
  const handleAddComment = async (id, userId, comment, rating) => {
    const data = {
      user: userId,
      comment: comment,
      rating: rating,
    ...
    try {
      ...
      let updatedComments = albumData.comments || [];
      ...
      await editData(id, updatedAlbum);
  ...
  const handleFilter = (searchTerm, searchGenre, sortByYear) => {
    let filtered = [...data];
    ...
    if (searchGenre) {
      filtered = filtered.filter((album) =>
        album.genre.toLowerCase().includes(searchGenre.toLowerCase())
      );
    }

    if (sortByYear) {
      filtered.sort((a, b) => (sortByYear ? a.year - b.year : b.year - a.year));
    }

    setFilteredData(filtered);
  };

  const handleAddAlbumModalOpen = () => {
    ...
    if (confirmDelete) {
      try {
        ...
        setFilteredData((prevFilteredData) =>
          prevFilteredData.filter((product) => product.id !== id)
        );
          ...
          <Toolbar>
            ...
            {user ? (
              <>
                ...
                {user && user.isAdmin ? (
                  <Button color="inherit" onClick={handleAddAlbumModalOpen}>
                    Add
                    <AddAlbumForm
                      open={addAlbumModalOpen}
                      handleClose={handleAddAlbumModalClose}
                      setData={setData}
                      data={data}
                    />
                  </Button>
                ) : (
                  <Button color="inherit" onClick={handleWishlistModalOpen}>
                    <WishlistModal
                      user={user}
                      data={data}
                  ...
                  <CardActions>
                    {user && canEditAndDelete(user) ? (
                      <>
                        <Button
                          size="small"
                          onClick={() => handleDeleteAlbum(card.id)}
                        >
                          Delete
                        </Button>
                        <Button
